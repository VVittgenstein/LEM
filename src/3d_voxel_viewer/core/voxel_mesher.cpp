...
struct ViewFrustum {
    ...
    static ViewFrustum Extract(const Camera3D& camera, float aspect, float far_plane = 50000.0f) {
        ...
        Matrix matProj = MatrixPerspective(camera.fovy * DEG2RAD, aspect, 0.5f, far_plane);
...
void VoxelMesher::MeshChunk(const VoxelGrid& grid,
                            int x0, int x1,
                            int z0, int z1,
            ...
            for (int slice = 0; slice < D; ++slice) {
                ...
                for (int iv = 0; iv < V; ++iv) {
                    for (int iu = 0; iu < U;) {
                        ...
                        if (!quad.is_water) {
                            quad.ao[0] = CalculateVertexAO(grid, start.voxel_x, start.voxel_y, start.voxel_z, d, sign, 0, 0);
                            quad.ao[1] = CalculateVertexAO(grid, start.voxel_x, start.voxel_y, start.voxel_z, d, sign, 1, 0);
                            quad.ao[2] = CalculateVertexAO(grid, start.voxel_x, start.voxel_y, start.voxel_z, d, sign, 1, 1);
                            quad.ao[3] = CalculateVertexAO(grid, start.voxel_x, start.voxel_y, start.voxel_z, d, sign, 0, 1);
                        } else {
                            quad.ao[0] = 3; quad.ao[1] = 3; quad.ao[2] = 3; quad.ao[3] = 3;
                        }
...
bool VoxelMesher::BuildMesh(const VoxelGrid& grid, const PaletteManager& palette) {
    ...
    for (int c = 0; c < total_chunks; ++c) {
        ...
        MeshChunk(grid, x0, x1, z0, z1, chunk_terrain[c], chunk_water[c]);
    }
    ...
    stats.optimized_quads = static_cast<int>(terrain_quads.size() + water_quads.size());
    stats.terrain_triangles = static_cast<int>(terrain_quads.size() * 2);
    stats.water_triangles = static_cast<int>(water_quads.size() * 2);
    ...
    int total_t_quads = static_cast<int>(terrain_quads.size());
    for (int i = 0; i < total_t_quads; i += MAX_QUADS_PER_SUBMESH) {
        int count = std::min(MAX_QUADS_PER_SUBMESH, total_t_quads - i);
        BoundingBox bb{};
    ...
    int total_w_quads = static_cast<int>(water_quads.size());
    for (int i = 0; i < total_w_quads; i += MAX_QUADS_PER_SUBMESH) {
        int count = std::min(MAX_QUADS_PER_SUBMESH, total_w_quads - i);
        BoundingBox bb{};
...
void VoxelMesher::UpdateColors(const VoxelGrid& grid, const PaletteManager& palette) {
    if (has_terrain_model && terrain_model.meshCount > 0) {
        ...
        for (int m_idx = 0; m_idx < terrain_model.meshCount; ++m_idx) {
            ...
            int start_quad = m_idx * MAX_QUADS_PER_SUBMESH;
            int num_quads = std::min(MAX_QUADS_PER_SUBMESH, total_t_quads - start_quad);
