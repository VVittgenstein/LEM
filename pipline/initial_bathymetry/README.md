...
- 新增代码放置于 `F:\LEM\pipline\initial_bathymetry`，以等高线输出图。用户在确认这些要求后明确指示“可以开始”。
...
| 坡折与坡度观测 | `datasets/2026-09-09-q1-pretasks/F/stations/*.npz`中的冻结剖面 |
...
```powershell
& F:\LEM\pipline\initial_bathymetry\run.ps1
& F:\LEM\pipline\initial_bathymetry\run.ps1 -Stage prepare
& F:\LEM\pipline\initial_bathymetry\run.ps1 -Stage generate
& F:\LEM\pipline\initial_bathymetry\run.ps1 -Stage audit
& F:\LEM\pipline\initial_bathymetry\run.ps1 -Stage test
& F:\LEM\pipline\initial_bathymetry\run.ps1 -Stage render
```
