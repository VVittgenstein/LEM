...
```powershell
& F:\LEM\pipline\mask_generator\run_world_orogen_gibbs.ps1
```
...
```powershell
& F:\LEM\pipline\mask_generator\run.ps1
```
...
```powershell
& F:\LEM\pipline\mask_generator\run.ps1 -Stage test
& F:\LEM\pipline\mask_generator\run.ps1 -Stage prepare
& F:\LEM\pipline\mask_generator\run.ps1 -Stage calibrate -Workers 8
& F:\LEM\pipline\mask_generator\run.ps1 -Stage generate
& F:\LEM\pipline\mask_generator\run.ps1 -Stage audit
& F:\LEM\pipline\mask_generator\run.ps1 -Stage report
```
...
重复执行会覆盖本生成器同名产物。已有输出目录必须含本程序的身份标记 `.mask_generator.json`；程序拒绝写入其他已有内容的目录。`-Output` 可用于另存一批；`-PythonPath` 可指定具有同等依赖的现有隔离环境。计算代码、测试和启动脚本全部在本目录，运行时不导入旧生成器或其他会话代码。
...
| 原始矢量 | `datasets/2026-09-08-q1-data/D/gshhs_f.b`，GSHHG 2.3.7 full-resolution level-1 外海岸 |
| 原尺度参考栅格 | `datasets/2026-09-08-q1-data/D/derived-1km-masks/<id>.npz` 中的 `focal`，用于方法 A 的参考统计 |
