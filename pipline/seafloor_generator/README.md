...
3. 参考数据：`datasets/2026-09-09-q1-pretasks/F/stations/*.npz`，读取原F程序保存的1至250 km逐公里剖面。参考站点与原始数据均只读。
4. 复用代码：`pipline/mask_generator/common/io.py`、`common/render.py` 和 `world_orogen_gibbs/geometry.py`。后者继续使用既有分区数据结构及邻接计算。本程序读取现有分区，不重新运行分区生长或 Gibbs 选择。
...
```powershell
& F:\LEM\pipline\seafloor_generator\run.ps1
& F:\LEM\pipline\seafloor_generator\run.ps1 -Stage prepare
& F:\LEM\pipline\seafloor_generator\run.ps1 -Stage generate
& F:\LEM\pipline\seafloor_generator\run.ps1 -Stage audit
& F:\LEM\pipline\seafloor_generator\run.ps1 -Stage test
```
