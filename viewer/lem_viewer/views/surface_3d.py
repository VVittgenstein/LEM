...
from lem_viewer.semantics import KIND_DRAINAGE_AREA, KINDS, find_elevation_channel, elevation_in_metres
...
def build_surface(dataset: TerrainDataset, channel_name: str, *, max_display_size: int = 512,
    ...
    if height_ch is not None:
        heights = (elevation_in_metres(height_ch) if height_ch.kind == "elevation" or height_ch.name == "elevation"
                   else np.asarray(height_ch.get_array(), dtype=float))
        z = downsample_for_display(heights, max_display_size)
    else:
    ...
    stride = max(1, int(np.ceil(max(full_rows, full_cols) / max_display_size)))
    step_y = dy * stride
    step_x = dx * stride
...
def make_gl_view(dataset: TerrainDataset, channel_name: str, **kwargs: Any) -> tuple[SyncableGLView, BandedScale, TerrainChannel, float]:
    ...
    ex, ey, ez = extents
    center_z = ez / 2.0
    if _height_ch is not None:
        raw = (elevation_in_metres(_height_ch) if _height_ch.kind == "elevation" or _height_ch.name == "elevation"
               else np.asarray(_height_ch.get_array(), dtype=float))
        finite = raw[np.isfinite(raw)]
        if finite.size:
            center_z = float((finite.min() + finite.max()) * 0.5 * vex)
    view.setCameraPosition(pos=Vector(ex / 2.0, ey / 2.0, center_z), distance=float(max(ex, ey) * 1.3),
