...
void HUD::Draw(const CameraController& cam,
    ...
    // Each Qt display can be narrow. Keep the original full overlay for wide
    // windows, and use a compact version of the same measurements otherwise.
    if(screen_width<900) {
        const int width=std::max(120,screen_width-16);
        const int height=show_profiler?230:100;
        DrawRectangle(8,8,width,height,Fade(BLACK,.80f));
        char compact[160]; int y=18;
        auto line=[&](const char* text,Color color=RAYWHITE){DrawText(text,16,y,12,color);y+=17;};
        std::snprintf(compact,sizeof(compact),"%d FPS | %.2f ms",GetFPS(),profile.t_frame_total_ms);line(compact,LIME);
        line(cam.mode==CAM_ORBIT?"Camera: Orbit":"Camera: Fly");
        std::snprintf(compact,sizeof(compact),"Grid: %d x %d | levels: %d",grid.size_x,grid.size_z,grid.size_y-4);line(compact);
        std::snprintf(compact,sizeof(compact),"Quads: %" PRIu64,mesher.stats.optimized_quads);line(compact);
        if(show_profiler){
            std::snprintf(compact,sizeof(compact),"Visible meshes: %d / %d",profile.visible_submeshes,profile.total_submeshes);line(compact);
            std::snprintf(compact,sizeof(compact),"Input: %.1f us",profile.t_input_us);line(compact);
            std::snprintf(compact,sizeof(compact),"Camera: %.1f us",profile.t_camera_us);line(compact);
            std::snprintf(compact,sizeof(compact),"Culling: %.1f us",profile.t_culling_us);line(compact);
            std::snprintf(compact,sizeof(compact),"Draw: %.2f ms",profile.t_render3d_us/1000);line(compact);
            std::snprintf(compact,sizeof(compact),"Overlay: %.1f us",profile.t_hud_us);line(compact);
            std::snprintf(compact,sizeof(compact),"Present: %.2f ms",profile.t_present_us/1000);line(compact);
        }
        return;
    }
    ...
    std::snprintf(buf, sizeof(buf), "Solid Voxels: %" PRIu64 " | Water: %" PRIu64,
                  mesher.stats.total_solid_voxels, grid.CountWaterVoxels());
    DrawText(buf, panel_x + 12, y_cursor, 13, LIGHTGRAY);
    y_cursor += 16;

    std::snprintf(buf, sizeof(buf), "Faces: %" PRIu64 " -> %" PRIu64 " Quads (-%.2f%%)",
    ...
    std::snprintf(buf, sizeof(buf), "Triangles: %" PRIu64 " | Frustum Culling: %s",
