...
bool PaletteManager::LoadFromIni(const std::string& filepath) {
    ...
    while (std::getline(file, line)) {
        ...
        if (line.find_first_not_of(" \t") != std::string::npos && line[line.find_first_not_of(" \t")]=='#') continue;
...
const char* PaletteManager::GetCurrentSchemeName() const {
    switch (current_scheme) {
        case SCHEME_BIOME: return "Material appearance";
        ...
        case SCHEME_CHANNEL: return "Data channel";
...
Color PaletteManager::GetFaceColor(uint8_t voxel_type,
    ...
    if (voxel_type == VOXEL_WATER) {
        ...
        switch (current_scheme) {
            case SCHEME_CHANNEL:
                base_color = Color{uint8_t(col.display_color), uint8_t(col.display_color>>8),
                                   uint8_t(col.display_color>>16), uint8_t(col.display_color>>24)};
                break;
            case SCHEME_BIOME: {
                switch (voxel_type) {
