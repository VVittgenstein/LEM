...
void VoxelGrid::ConfigurePhysical(const FastscapeData& data, double dx, double dy,
                                  int levels, double exaggeration) {
    if(levels<2 || !std::isfinite(dx) || !std::isfinite(dy) || dx<=0 || dy<=0 ||
       !std::isfinite(exaggeration) || exaggeration<=0) throw std::runtime_error("Invalid physical scale");
    physical_coordinates=true;
    scene_unit_m=std::max(dx,dy);
    cell_x=float(dx/scene_unit_m); cell_z=float(dy/scene_unit_m);
    double range=double(data.header.h_max)-data.header.h_min;
    double step=range>0.0001 ? range/(levels-1) : 1.0;
    height_step=float(step*exaggeration/scene_unit_m);
    height_origin=float(data.header.h_min*exaggeration/scene_unit_m)-height_step;
    floor_height=float((data.header.h_min-std::min(dx,dy))*exaggeration/scene_unit_m);
    water_surface=float(data.header.water_level*exaggeration/scene_unit_m);
    double index=(double(data.header.water_level)-data.header.h_min)/step;
    water_layer=data.header.has_water ? int(std::clamp(std::round(index),-1.0,double(levels))) : -1;
}

float VoxelGrid::WorldHeight(float y, bool water) const {
    if(!physical_coordinates) return y;
    if(y==0) return floor_height;
    if(water && y==float(water_layer+1)) return water_surface;
    return height_origin+y*height_step;
}

float VoxelGrid::GroundHeightWorldAt(float x, float z) const {
    if(!physical_coordinates) return float(GetGroundHeightAt(int(x),int(z)));
    int ix=int(std::floor(x/cell_x+0.5f)), iz=int(std::floor(z/cell_z+0.5f));
    if(ix<0 || iz<0 || ix>=size_x || iz>=size_z) return -std::numeric_limits<float>::infinity();
    int ground=GetColumn(ix,iz).ground_height;
    return ground<0 ? -std::numeric_limits<float>::infinity() : WorldHeight(float(ground+1));
}

size_t VoxelGrid::CheckedVoxelCount(int sx, int sy, int sz) {
    if(sx<=0 || sy<=0 || sz<=0)throw std::invalid_argument("Voxel dimensions must be positive");
    size_t count=size_t(sx);
    for(int dimension:{sy,sz}){
        if(size_t(dimension)>std::numeric_limits<size_t>::max()/count)
            throw std::length_error("Voxel dimensions exceed addressable memory");
        count*=size_t(dimension);
    }
    if(count>std::vector<uint8_t>().max_size() ||
       size_t(sx)*size_t(sz)>std::vector<ColumnAttributes>().max_size())
        throw std::length_error("Voxel dimensions exceed container capacity");
    return count;
}

void VoxelGrid::Allocate(int sx, int sy, int sz) {
    size_t count=CheckedVoxelCount(sx,sy,sz);
    std::vector<uint8_t> next_voxels(count,VOXEL_AIR);
    std::vector<ColumnAttributes> next_columns(size_t(sx)*size_t(sz));
    voxels.swap(next_voxels);column_attrs.swap(next_columns);
    size_x = sx;
    size_y = sy;
    size_z = sz;
}
...
bool VoxelGrid::QuantizeFromFastscape(const FastscapeData& data,
    ...
    for (int z = 0; z < sz; ++z) {
        for (int x = 0; x < sx; ++x) {
            if(!std::isfinite(data.GetElevation(x,z))) {
                column_attrs[GetColumnIndex(x,z)].ground_height=-1;
                continue;
            }
            ...
            size_t col_idx = GetColumnIndex(x, z);
    ...
    for (int z = 0; z < sz; ++z) {
        for (int x = 0; x < sx; ++x) {
            ...
            bool is_steep = (float(max_step)*35.0f/(vertical_voxels-1) >= 2.0f);
            int dirt_layers=std::max(1,int(std::round(3.0f*(vertical_voxels-1)/35.0f)));

            // Fill ground voxels from bottom y=0 to gy
            for (int y = 0; y <= gy; ++y) {
                if (y == 0) {
                    SetVoxel(x, y, z, VOXEL_BEDROCK);
                } else if (y < gy - dirt_layers) {
                    ...
                    } else if (float(gy)/(vertical_voxels-1) >= 36.0f*0.88f/35.0f) {
                        SetVoxel(x, y, z, VOXEL_SNOW); // High alpine snow
                    } else if (is_steep || float(gy)/(vertical_voxels-1) >= 36.0f*0.70f/35.0f) {
...
uint64_t VoxelGrid::CountSolidVoxels() const {
    uint64_t count = 0;
...
uint64_t VoxelGrid::CountWaterVoxels() const {
    uint64_t count = 0;
    for (uint8_t v : voxels) {
        if (v == VOXEL_WATER) {
            count++;
