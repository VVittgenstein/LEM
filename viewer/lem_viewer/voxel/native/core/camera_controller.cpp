...
void CameraController::Setup(Vector3 terrain_center, float max_dim, float max_ground_y, float center_ground_y, const VoxelGrid* grid) {
    ...
    orbit_target = Vector3{terrain_center.x, center_ground_y + (grid && grid->physical_coordinates ? 0.0f : 2.0f), terrain_center.z};
    ...
    float cell_size=grid?std::min(grid->cell_x,grid->cell_z):1.0f;
    min_orbit_distance = std::max(0.005f,cell_size*0.05f);
    ...
    orbit_distance = std::clamp(max_dim * 1.85f/std::min(1.f,std::max(.1f,aspect)), min_orbit_distance, max_orbit_distance);
...
void CameraController::ApplyCustomProjection(float aspect) const {
    ...
    double top = near_plane * std::tan(camera.fovy * 0.5 * DEG2RAD);
    double right = top * aspect;
    rlFrustum(-right, right, -top, top, near_plane, far_plane_distance);
    rlMatrixMode(RL_MODELVIEW);
}

void CameraController::EnsureAboveGround(float margin) {
    if (grid_ref != nullptr) {
        float gy = grid_ref->GroundHeightWorldAt(camera.position.x,camera.position.z);
        if(grid_ref->physical_coordinates) margin=0.02f*std::min(grid_ref->cell_x,grid_ref->cell_z);
...
void CameraController::UpdateOrbit(float dt) {
    ...
    if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) && !ViewerKeyDown(KEY_LEFT_SHIFT)) {
    ...
    if (IsMouseButtonDown(MOUSE_BUTTON_MIDDLE) || IsMouseButtonDown(MOUSE_BUTTON_RIGHT) ||
        (IsMouseButtonDown(MOUSE_BUTTON_LEFT) && ViewerKeyDown(KEY_LEFT_SHIFT))) {
...
void CameraController::Zoom(float wheel) {
    // Multiplicative steps stay positive with fast wheels and trackpads.
    float factor=std::pow(0.9f,std::clamp(wheel,-100.f,100.f));
    if(mode==CAM_FLYCAM){
        camera.fovy=std::clamp(camera.fovy*factor,1.f,90.f);
        return;
    }
    orbit_distance = std::clamp(orbit_distance * factor,
...
void CameraController::UpdateFlycam(float dt) {
    ...
    if (IsMouseButtonDown(MOUSE_BUTTON_RIGHT) || IsMouseButtonDown(MOUSE_BUTTON_LEFT)) {
        float sensitivity=mouse_sensitivity*camera.fovy/45.f;
        fly_yaw -= mouse_delta.x * sensitivity;
        fly_pitch -= mouse_delta.y * sensitivity;
        fly_pitch = std::clamp(fly_pitch, -1.50f, 1.50f);
    }

    // Optical zoom keeps the flight position and movement speed unchanged.
    float wheel = GetMouseWheelMove();
    if (wheel != 0.0f) Zoom(wheel);

    // 10x SPRINT BOOST: Holding Left or Right Control multiplies movement speed by 10
    is_sprinting = ViewerKeyDown(KEY_LEFT_CONTROL) || ViewerKeyDown(KEY_RIGHT_CONTROL);
    ...
    if (ViewerKeyDown(KEY_W)) move_dir = Vector3Add(move_dir, forward_h);
    if (ViewerKeyDown(KEY_S)) move_dir = Vector3Subtract(move_dir, forward_h);
    if (ViewerKeyDown(KEY_D)) move_dir = Vector3Add(move_dir, right_h);     // D: move RIGHT
    if (ViewerKeyDown(KEY_A)) move_dir = Vector3Subtract(move_dir, right_h);  // A: move LEFT

    // Vertical movement: Space = Up (+Y), Left Shift = Down (-Y)
    // Control is exclusively dedicated to x10 Sprint Boost
    if (ViewerKeyDown(KEY_SPACE)) move_dir.y += 1.0f;
    if (ViewerKeyDown(KEY_LEFT_SHIFT)) move_dir.y -= 1.0f;
