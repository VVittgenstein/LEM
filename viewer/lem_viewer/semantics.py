...
def elevation_in_metres(channel) -> np.ndarray:
    """Convert declared elevation units for geometry and derived physical slopes."""
    unit = channel.units.strip().lower()
    factor = {"": 1.0, "m": 1.0, "meter": 1.0, "meters": 1.0,
              "metre": 1.0, "metres": 1.0, "km": 1000.0, "cm": .01, "mm": .001}.get(unit)
    if factor is None:
        raise ValueError(f"Unsupported elevation unit: {channel.units}")
    return np.asarray(channel.get_array(), dtype=float) * factor
