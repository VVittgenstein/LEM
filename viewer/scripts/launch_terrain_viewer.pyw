...
def repo_root() -> Path:
    return next(p for p in Path(__file__).resolve().parents if (p / 'pyproject.toml').is_file() and (p / 'AGENTS.md').is_file())


def ensure_viewer_on_path() -> Path:
    """Make the repo-local ``viewer/`` importable without editable install."""

    src_dir = repo_root() / "viewer"
