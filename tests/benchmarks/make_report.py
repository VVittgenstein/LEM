...
RES = (next(p for p in Path(__file__).resolve().parents if (p / 'pyproject.toml').is_file() and (p / 'AGENTS.md').is_file())) / "bench/results" / "L1"
...
def main():
    ...
    smoke = {}
    for cfg, _, _ in CONFIGS:
        p = (next(p for p in Path(__file__).resolve().parents if (p / 'pyproject.toml').is_file() and (p / 'AGENTS.md').is_file())) / "bench/results" / "_smoke" / f"r0_{cfg}.json"
        if p.exists():
    ...
    env = json.loads(((next(p for p in Path(__file__).resolve().parents if (p / 'pyproject.toml').is_file() and (p / 'AGENTS.md').is_file())) / "bench/results" / "env_check.json").read_text(encoding="utf-8"))
