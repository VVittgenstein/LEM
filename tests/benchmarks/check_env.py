...
OUT = (next(p for p in Path(__file__).resolve().parents if (p / 'pyproject.toml').is_file() and (p / 'AGENTS.md').is_file())) / "bench/results" / "env_check.json"
...
try:
    p = (next(p for p in Path(__file__).resolve().parents if (p / 'pyproject.toml').is_file() and (p / 'AGENTS.md').is_file())) / "bench/results" / "_zarr_check.zarr"
    if p.exists():
        shutil.rmtree(p)
