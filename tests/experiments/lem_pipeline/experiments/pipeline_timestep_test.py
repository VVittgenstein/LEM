...
Examples (lem-env interpreter, from the repository root):
  PYTHONPATH=experiments lem-env/python.exe tests/experiments/lem_pipeline/experiments/pipeline_timestep_test.py run --root output/timestep_test --workers 8 --t-end 4e6
  PYTHONPATH=experiments lem-env/python.exe tests/experiments/lem_pipeline/experiments/pipeline_timestep_test.py analyze --root output/timestep_test
"""
...
from tests.experiments.lem_pipeline.experiments import timestep_test as tt  # noqa: E402
...
def main() -> None:
    ...
    if a.mode in ("run", "jobs"):
        ...
        results = run_jobs("tests.experiments.lem_pipeline.experiments.timestep_test.run_case", jobs, max_workers=a.workers, record_path=root / "jobs-record.json")
