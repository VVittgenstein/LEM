...
Run with the lem-env interpreter: PYTHONPATH=src lem-env/python.exe scripts/pipeline_population34.py
"""
...
日期：{datetime.now(timezone.utc).strftime('%Y-%m-%d')}。生成脚本：`scripts/pipeline_population34.py`（`src/lem_pipeline/reference.py`）。总体：D 会话核定的 71 个大陆地壳型陆块中 `abs(latitude) < 60` 的 34 个（yZz 2026-09-09 决定）。输入为 A 会话的逐陆块 JSON（`A-initial-elevation-sealevel/tables/spatial-by-landmass/`，{len(landmasses)} 个陆块有文件）与 D 会话的逐陆块表。全部行的依据状态为“本项目借鉴或合成改造”；本重汇总不构成验收。
