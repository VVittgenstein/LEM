...
(one row per landmass) and reference-land-statistics.json (pooled quantiles, cell-weighted). Run:
  PYTHONPATH=src lem-env/python.exe scripts/pipeline_landstats.py [--limit N]
"""
...
def main() -> None:
    ...
    summary = {"created_utc": datetime.now(timezone.utc).isoformat(), "population": 34, "n_ok": sum(1 for r in rows if r.get("status") == "ok"),
               "m": M, "n": N, "source": "datasets/2026-09-08-q1-data/A/subsets (GEBCO 2026 30 arc-second), richdem D8 after epsilon depression filling",
