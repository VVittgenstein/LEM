...
def main():
    ...
    if args.stage in ('all','test'):
        ...
        suite=unittest.defaultTestLoader.discover(str(next(p for p in Path(__file__).resolve().parents if (p / 'pyproject.toml').is_file() and (p / 'AGENTS.md').is_file()) / 'tests/data_generation/stage/mask_generator/world_orogen_gibbs'),top_level_dir=str(next(p for p in Path(__file__).resolve().parents if (p / 'pyproject.toml').is_file())))
