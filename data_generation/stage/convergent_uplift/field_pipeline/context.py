...
REPO = next(p for p in Path(__file__).resolve().parents if (p / 'pyproject.toml').is_file() and (p / 'AGENTS.md').is_file())
RAW = ROOT / 'sources'
MODELS = ROOT / 'models'
CHECKS = ROOT / 'checks'
OUT = ROOT / 'output'
...
def freeze():
    initialize()
    existing=RAW/'manifest.json'
    if existing.exists():
        frozen=jload(existing)
        for record in frozen['files']:
            target=ROOT/record['copy']
            if sha(target)!=record['sha256']:raise ValueError(f'Frozen source changed: {target}')
        return frozen['files']
    ...
    pairs = [(case/'sources/C03/2024JB030625_DS5_ENU_velocity.txt', RAW/'DS5_ENU_velocity.txt'),
             ...
             (REPO/'viewer/lem_viewer/colormaps.py', RAW/'viewer_colormaps.py')]
    for ext in ('shp','shx','dbf','prj','cpg'):
        p=REPO/f'references/data/2026-09-08-q1-data/D/global_gprv.{ext}'
