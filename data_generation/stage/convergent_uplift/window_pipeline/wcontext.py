...
def recorded_path(value):
    """Resolve paths recorded before the directory migration without rewriting records."""
    path=Path(value)
    previous=REPO/'pipline'
    if path.is_relative_to(previous):
        return REPO/'data_generation/stage'/path.relative_to(previous)
    return path
def csvsave(p,rows):
    rows=list(rows)
